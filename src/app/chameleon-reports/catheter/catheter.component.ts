...
@Component({
  ...
  styleUrls: ['./catheter.component.scss']
})
export class CatheterComponent implements OnInit {
   ...
   Title1: string = '  רשימת צנתרים  - ';
   Title2: string = 'סה"כ תוצאות   ';
   titleUnit: string = 'צנתרים ';
   ...
   ngOnInit() {
     ...
     this.http
       .get<any[]>('http://localhost:7144/api/catheterAPI')
       .subscribe((data) => {
   ...
   fetchAnswerTextOptions() {
     this.http
       ...
       .subscribe((data) => {
         ...
         this.answerTextOptions = [...new Set(data.map((item) => item.ssP_DESCRIPTION))];
